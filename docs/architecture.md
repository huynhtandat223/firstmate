...
For an ordinary crew, the normal-mode watcher surfaces one stale wake when the pause begins, carrying the pause reason rather than a bare stale line, then applies that same cadence to an unchanged `paused:` or durable `captain-held` endpoint for the rest of that pause, whether or not the backend reports its agent dead.
