...
The watcher has no native Orca busy signal, so the busy-state contract in [architecture](architecture.md#busy-state-is-semantic-per-adapter) supplies harness-specific lifecycle and fallback classification.
