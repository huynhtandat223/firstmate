...
FM_WORK_EVIDENCE_FILE_SECS=240     # defaults to FM_STALE_ESCALATE_SECS; how recently a file git reports as changed in the task worktree must have been modified to count as work evidence that holds the wedge escalation open
FM_WORK_EVIDENCE_MAX_SECS=3600     # defaults to FM_BUSY_TURN_MAX_SECS; the longest work evidence may hold one stale episode's wedge escalation open, measured from the first extension; past it a stale pane escalates on the plain FM_STALE_ESCALATE_SECS cadence however alive its processes look
...
FM_BUSY_REGEX=          # optional override for rendered delivery guards and the isolated task-state fallbacks; converted worker state ignores it
