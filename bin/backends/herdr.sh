...
EOF
      case "$agent:$agent_status" in
        agy:idle|agy:done|agy:blocked)
          stripped=$(printf '%s\n' "$FM_BACKEND_HERDR_AGY_ROW_RAW" | fm_composer_strip_ghost)
          fm_composer_classify_content 1 "$stripped" "${FM_COMPOSER_IDLE_RE:-$FM_COMPOSER_IDLE_RE_DEFAULT}" insensitive "$FM_BACKEND_HERDR_AGY_ROW_RAW"
          return 0
          ;;
      esac
    fi
...
# fm_backend_herdr_payload_in_composer: answer whether the bounded composer
# capture still holds the payload this caller typed. This is a delivery proof,
# not an injection-safety verdict: the caller asks only after typing the payload,
# so a working Pi or identity-gated agy can be located without weakening the
# shared classifier's strict empty-composer rules.
fm_backend_herdr_payload_in_composer() {  # <target> <payload> -> yes|no|unreadable
  local target=$1 payload=$2 cap caps content probe
  if cap=$(fm_backend_herdr_capture_ansi "$target" "$FM_COMPOSER_CAPTURE_LINES" 2>/dev/null); then
    caps=$(printf 'styled=1\ncursor=0\nidentity=1\nrows=%s' "$FM_COMPOSER_CAPTURE_LINES")
  elif cap=$(fm_backend_herdr_capture "$target" "$FM_COMPOSER_CAPTURE_LINES"); then
    caps=$(printf 'styled=0\ncursor=0\nidentity=1\nrows=%s' "$FM_COMPOSER_CAPTURE_LINES")
  ...
  content=$(fm_composer_extract_selected_content "$caps" "$cap" 2>/dev/null || true)
  if [ -z "$content" ]; then
    fm_backend_herdr_agy_composer_find "$cap"
    if [ "$FM_BACKEND_HERDR_AGY_ROW_FOUND" -eq 1 ]; then
      content=$(printf '%s\n' "$FM_BACKEND_HERDR_AGY_ROW_RAW" | fm_composer_strip_ansi)
    fi
  fi
  [ -n "$content" ] || { printf 'no'; return 0; }
  probe=$(printf '%s' "$payload" | cut -c1-24)
  [ -n "$probe" ] || { printf 'unreadable'; return 0; }
  case "$content" in *"$probe"*) printf 'yes' ;; *) printf 'no' ;; esac
}
...
fm_backend_herdr_send_text_submit() {  # <target> <text> <retries> <enter-sleep> <settle>
  ...
  local raw_status footer_baseline='' anchored=0
  fm_backend_herdr_parse_target "$target" || { printf 'unknown'; return 0; }
  ...
  # Typing never starts a turn, so reads taken after the literal send and before
  # the first Enter are still pre-submission baselines. When our payload is
  # structurally visible, its disappearance is the strongest busy-baseline
  # proof and works for Pi and agy whose injection classifier must refuse while
  # they are working.
  if [ "$raw_status" = working ] \
     && [ "$(fm_backend_herdr_payload_in_composer "$target" "$text")" = yes ]; then
    anchored=1
  fi
  if [ "$baseline" != idle ] && [ "$anchored" != 1 ]; then
