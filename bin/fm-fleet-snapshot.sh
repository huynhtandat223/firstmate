...
# Staged on disk before any consumer runs: a real home's backlog JSON is far larger than a command
# line can carry, so every jq below reads it with --slurpfile rather than --argjson.
