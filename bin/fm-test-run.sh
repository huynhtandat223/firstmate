...
family_for_basename() {
  case "$1" in
    ...
    fm-operational-input.test.sh|fm-orchestrator-assistance.test.sh|fm-paired-review-writing-pointer.test.sh|\
    fm-pi-primary-types.test.sh|fm-send-popup-settle.test.sh|fm-send-settle.test.sh|\
    ...
    fm-session-lock-liveness.test.sh|\
    ...
    fm-spawn-agent-path.test.sh|fm-spawn-dispatch-profile.test.sh|\
    ...
    fm-backend-orca.test.sh)
      printf '%s\n' orca
      ;;
    fm-pair-compose.test.sh|fm-temporary-supervisor.test.sh)
      printf '%s\n' unclassified
      ;;
    *)
      printf '%s\n' unclassified
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-gate-refuse*|bin/fm-lock*|bin/fm-quota-axi-lib.sh)
      printf '%s\n' session-bootstrap
      ;;
    bin/fm-sessionstart-run.sh|bin/fm-assistance-primary-session.sh|\
    ...
    bin/fm-composer-lib.sh)
      ...
      printf '%s\n' live-harness-optin
      ;;
    bin/fm-spawn.sh|bin/fm-send.sh|bin/fm-harness.sh|bin/fm-procevent-assistance.sh|\
