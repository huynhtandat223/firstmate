...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  # temporary supervisors delegate delivery, and secondmate spawns hardcode theirs.
  if [ "$KIND" = ship ]; then
    [ "$MODE_SET" -eq 1 ] || {
...
# worktree is its firstmate home, so neither takes a task worktree nor arms
# task-pane wiring.
OWN_HOME=0
...
# muse is verified as a CREWMATE/SCOUT adapter only. Home-owning agents need a
# primary supervision protocol; muse has none.
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  T=$RELAUNCH_TARGET
  # A home-owning task already resolved WT above through its home validation;
  # every ordinary task takes the recorded task worktree.
  [ "$OWN_HOME" -eq 1 ] || WT=$RELAUNCH_WT
  WT_TARGET=$T
  SES=${T%%:*}
else
case "$BACKEND" in
