...
for a in "$@"; do
  ...
  case "$a" in
    ...
    --supervisor) KIND=supervisor; KIND_SET=1 ;;
...
# reason asks it instead of naming a kind twice. It is derived from KIND alone,
# so it resolves here rather than with the deferred backend selection below.
OWN_HOME=0
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  [ -n "$KIND" ] || KIND=ship
  # The adopted kind replaces the command-line one, so the home-owning shape has
  # to be re-derived from it. Every OWN_HOME branch below would otherwise read
  # the fresh-spawn default and treat a relaunched secondmate or temporary
  # supervisor as an ordinary task worktree.
  OWN_HOME=0
  fm_supervisor_kind_self_supervising "$KIND" && OWN_HOME=1
  ...
  if [ "$OWN_HOME" -eq 1 ]; then
    # A secondmate and a temporary supervisor both record home= rather than
    # project=, so the home-owning shape selects the adoption source here too.
...
effort_flag_for_harness() {
  ...
  case "$harness" in
    ...
    pi|pi-signed)
      ...
      ;;
    agy)
      # Antigravity CLI accepts --effort low|medium|high.
      case "$effort" in
...
elif [ "$OWN_HOME" -eq 0 ] && [ "$BACKEND" != orca ]; then
