...
- **Planner** produces a captain-approved spec or tickets through a session the captain talks to directly; read `.agents/roles/planner/contract/SKILL.md` before dispatching a planner session when the captain invokes it or asks to plan, scope, spec, or break down work before implementation is authorized. Its artifact never authorizes implementation, which stays a separate captain-authorized lifecycle.
- **Orchestrator** drives an already captain-authorized spec, ticket set, or GitHub issues to completion through a session the captain talks to directly, which dispatches its own workers; read `.agents/roles/orchestrator/contract/SKILL.md` before dispatching an orchestrator session when the captain invokes it or asks to run or take over such a programme. Its input is that authorized work rather than the backlog.
