...
| `/orchestrator`    | Open an orchestrator session you talk to directly: it drives an already-authorized spec, ticket set, or GitHub issues to completion through implementation workers it dispatches and supervises itself, holding the view across every ticket at once |
| `/planner`         | Open a planner session you talk to directly: it investigates one scoped subject, grills you a question at a time with its own recommended answers, and publishes a spec or dependency-aware tickets only once you ask it to crystallize; the artifact never authorizes implementation |
