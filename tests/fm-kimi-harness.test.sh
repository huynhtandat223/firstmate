...
test_kimi_launch_then_send_is_verified() {
  ...
  [ "$launch" = "export CHROME_DEVTOOLS_AXI_SESSION=$id; export COMPACT_ADVISER_DISABLE=1; env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI '$FAKEBIN_DIR/kimi' --model 'kimi-code/k3' --auto" ] \
...
test_kimi_falls_back_to_expanded_home_binary() {
  ...
  launch=$(cat "$CASE_DIR/launch.log")
  [ "$launch" = "export CHROME_DEVTOOLS_AXI_SESSION=$id; export COMPACT_ADVISER_DISABLE=1; env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI '$fallback' --auto" ] \
