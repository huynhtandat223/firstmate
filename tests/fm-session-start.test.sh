...
# (Pi family), GROK_AGENT (grok), ANTIGRAVITY_AGENT (agy).
...
run_session_start() {
  ...
  if [ -n "$pi_harness" ]; then
    env -u CLAUDECODE -u GROK_AGENT -u ANTIGRAVITY_AGENT PI_CODING_AGENT=true FM_PI_HARNESS="$pi_harness" \
      FM_HOME="$home" FM_ROOT_OVERRIDE="$root" PATH="$path" \
      "$SESSION_START"
  else
    env -u CLAUDECODE -u PI_CODING_AGENT -u FM_PI_HARNESS -u GROK_AGENT -u ANTIGRAVITY_AGENT \
      FM_HOME="$home" FM_ROOT_OVERRIDE="$root" PATH="$path" \
      "$SESSION_START"
  fi
}
