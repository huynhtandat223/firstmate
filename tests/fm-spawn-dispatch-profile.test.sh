...
test_active_dispatch_profile_allows_raw_launch_command() {
  ...
  # so it carries the compact-adviser floor and its own browser session;
  # nothing else may rewrite the captain's own command.
  [ "$launch" = "export CHROME_DEVTOOLS_AXI_SESSION=$id; export COMPACT_ADVISER_DISABLE=1; custom-agent --flag" ] || fail "raw launch command changed"$'\n'"actual: $launch"
