...
test_send_text_submit_preexisting_working_does_not_false_confirm_swallowed_enter() {
  ...
  # 3: payload anchor read - the typed text is still in the composer
  # 4: send-keys enter; 5: payload read - the composer still holds the message
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/2.out"
  ...
  [ "$read_count" -eq 2 ] || fail "preexisting-working confirmation should anchor and then re-read the payload after Enter, made $read_count read(s)"
