...
test_large_backlog_survives_contribution_input() {
  local home note
  home=$(new_home large-backlog)
  record "$home" delivery 21 open mergeable
  # A real home's backlog is far larger than a single command-line argument may be, so this note alone
  # puts the parsed backlog JSON past that limit. Passing it through argv fails with E2BIG.
  note=$(head -c 300000 /dev/zero | tr '\0' 'n')
  printf '  %s\n' "$note" >> "$home/data/backlog.md"
  with_home "$home" "$ROOT/bin/fm-fleet-snapshot.sh" --contribution-input > "$home/input.json" \
    || fail 'a backlog larger than one command-line argument broke contribution input'
  jq -e '.backlog.present == true and (.backlog.records | length) >= 1 and (.tasks | type) == "array"' \
    "$home/input.json" >/dev/null \
    || fail 'contribution input from a large backlog was not usable'
  pass 'a backlog larger than one command-line argument still produces contribution input'
}

failures=0
for test_name in test_actor_coverage test_stale_verdict test_unchecked_is_not_silence test_newest_check_has_no_verdict test_comment_wake test_review_wake test_inline_wake test_ready_issue_wake test_fresh_issue_requires_maintainer test_missing_lane_remains_missing test_partial_freshness_keeps_measured_rows test_malformed_record_cannot_prove_silence test_issue_timeline_and_exact_ack test_verdict_retains_judged_head test_observed_replacement_refreshes_verdict test_unobserved_head_leaves_verdict_unknown test_away_yolo_is_fleet_work test_away_yolo_cross_home_is_fleet_work test_retired_and_unsupported_coverage test_unsupported_forge_is_not_fleet_work test_held_unsupported_forge_is_not_captain_work test_shared_contribution_signal_wakes_once test_watcher_keeps_diagnostics_separate_from_contribution_wakes test_expired_child_unsupported_forge_stays_unmeasured test_watcher_surfaces_new_contribution_once test_home_summary_coverage test_unreadable_pending_is_not_empty test_budget_refusal_between_calls test_budget_bounded_call_timeout test_genuine_failure_near_deadline_is_unavailable test_shared_url_observed_once test_terminal_contribution_settles test_late_owner_inherits_terminal_observation test_done_task_open_pr_still_observed test_failure_wakes_once_per_episode test_late_owner_keeps_failure_episode_suppressed test_large_backlog_survives_contribution_input; do
